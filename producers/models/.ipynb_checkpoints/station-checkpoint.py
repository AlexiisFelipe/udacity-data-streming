...
class Station(Producer):
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        self.producer.produce(topic=self.topic_name,
                              ...
                              value={"station_id": str(self.station_id),
                                     "train_id": str(train.train_id),
                                     "direction": direction,
                                     "line": self.color.name,
                                     "train_status": str(train.status),
                                     "prev_station_id": str(prev_station_id) if prev_station_id is not None else "---",
                                     "prev_direction": str(prev_direction) if prev_direction is not None else "---",},)
